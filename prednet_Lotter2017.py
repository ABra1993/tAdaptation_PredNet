...
class PredNet(Recurrent):
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        # get activation functions from Keras
        ...
        # setting the output (either image prediction or error)
        ...
        # set time when to start extrapolating
        self.extrap_start_time = extrap_start_time

        # define dimension for channel, height and width
    ...
    def step(self, a, states):
        ...
        for l in reversed(range(self.nb_layers)):
            ...
            # compute input
            inputs = [r_tm1[l], e_tm1[l]] # input to compute representations for layer l ant time t
            if l < self.nb_layers - 1:
                inputs.append(r_up)
        ...
        for l in range(self.nb_layers):
            ...
            if l == 0:
                ...
                # print('Frame prediction (max): ', K.min(frame_prediction))
        ...
        if self.output_layer_type is None: # if no specific layer type
            if self.output_mode == 'prediction':
