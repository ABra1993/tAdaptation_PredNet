...
def main():
    ...
    # output mode
    output_modes = ['E', 'R', 'A', 'Ahat']
    # output_modes = ['R', 'A', 'Ahat']
    ...
    # specify files for weights (can be found at https://github.com/coxlab/prednet.git)
    weights_file    = os.path.join('/weights/Lotter2017/tensorflow_weights/prednet_kitti_weights.hdf5')
    json_file       = os.path.join('/weights/Lotter2017/prednet_kitti_model.json')
    ...
    for output_mode in output_modes:
        if output_mode == 'error':
            ...
            for trial in trials:
                ...
                # X_test = np.load(root_stim + 'stimuli_' + trial + '_8_duration.npy')
            ...
            for trial in trials:
                ...
                # save
                if trained:
                    np.save(root_save + dataset + '/feature_maps/prednet_' + trial + '_' + output_mode + '_actvs_KITTI', metrics_avg)
                else:
                    np.save(root_save + dataset + '/feature_maps/prednet_' + trial + '_' + output_mode + '_actvs_random', metrics_avg)
            ...
            for trial in trials:
                ...
                for iL in range(n_layer):
                    ...
                    for iC in range(len(tempCond)): # iterate over temporal conditions (ISI or duration)
                        ...
                        metrics_avg_ch = metrics_avg.reshape(metrics_avg.shape[0], metrics_avg.shape[1], metrics_avg.shape[2], metrics_avg.shape[3]*metrics_avg.shape[4]*metrics_avg.shape[5]).mean(3)
                        print(metrics_avg_ch.shape)

                    # save
                    if trained:
                        np.save(root_save + dataset + '/avgs_CCN2025/prednet_' + trial + '_' + output_mode + str(iL+1) + '_actvs_KITTI', metrics_avg_ch)
                    else:
                        np.save(root_save + dataset + '/avgs_CCN2025/prednet_' + trial + '_' + output_mode + str(iL+1) + '_actvs_random', metrics_avg_ch)
