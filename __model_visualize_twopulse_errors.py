...
for iT, trial in enumerate(trials):
    ...
    for i, datasetstim in enumerate(dataset_stim):
        ...
        # retrieve data (temp. cond., n_img, nt, 4)
        temp = np.load(root_data + datasetstim + '/prednet_' + trial + '_' + output_mode + '_actvs_KITTI_8_duration.npy')

        # store data
        for iL in range(n_layer):
            if i == 0:
                data[iT, iL, :, :int(n_img/2), :] = temp[:, :, :, iL]
            else:
                data[iT, iL, :, int(n_img/2):, :] = temp[:, :, :, iL]
