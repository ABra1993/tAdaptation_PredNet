...
def plot_broadband(data, start, tempCond, n_layer, n_img, range_trans, range_sust, trained, population, dataset, output_mode, color_tempCond, color_layer, root_data, root_vis):
    ...
    fig.align_ylabels()
    plt.tight_layout()
    plt.savefig(root_vis + 'onepulse', dpi=600)
    # plt.savefig(root_vis + 'onepulse.svg')
    plt.close()
...
def plot_dynamics(dynamics_lin, dynamics_log, metric, tempCond, t1_plot, n_layer, color, n_img, init, computation, output_mode, root):
    ...
    plt.tight_layout()
    plt.savefig(root + 'onepulse_dynamics_fit', dpi=600)
    # plt.savefig(root + 'onepulse_dynamics_fit.svg')
    plt.close()
...
def plot_dynamics_metric(dynamics_fit, n_layer, computation, color, n_img, init, output_mode, root):
    ...
    plt.tight_layout()
    plt.savefig(root + 'onepulse_dynamics_fit_metric', dpi=600)
    # plt.savefig(root + 'onepulse_dynamics_fit_metric_.' + str(iInit+1) + '.svg')
    plt.close()
...
def plot_regression(ratio_lin_log, ratio_trans_sust, n_layer, color, n_img, init, model, root):
    ...
    for iL in range(n_layer):
        ...
        # if iL == 0:
        #     x_lim[0] = np.min(data_current_x)
        #     x_lim[1] = np.max(data_current_x)
        #     y_lim[0] = np.min(data_current_y)
        #     y_lim[1] = np.max(data_current_y)
        # else:

        #     min_value = np.min(data_current_x)
        #     max_value = np.max(data_current_x)
        #     if min_value < x_lim[0]:
        #         x_lim[0] = min_value
        #     if max_value > x_lim[1]:
        #         x_lim[1] = max_value

        #     min_value = np.min(data_current_y)
        #     max_value = np.max(data_current_y)
        #     if min_value < y_lim[0]:
        #         y_lim[0] = min_value
        #     if max_value > y_lim[1]:
        #         y_lim[1] = max_value
    ...
    # plot regression line
    x = ratio_lin_log[:, 0, :].mean(0).tolist() 
    y = ratio_trans_sust[:, 0, :].mean(0).tolist()
    ...
    # predict line
    y = model_LR.intercept_ + model_LR.coef_* np.linspace(np.min(x), np.max(x), 100)
    ax['regression'].plot(np.linspace(np.min(x), np.max(x), 100), y, color='crimson', lw=1, linestyle='dashed')

    ################################################################### L234

    # plot regression line
    x = ratio_lin_log[:, 1, :].mean(0).tolist() + ratio_lin_log[:, 2, :].mean(0).tolist() + ratio_lin_log[:, 3, :].mean(0).tolist()
    y = ratio_trans_sust[:, 1, :].mean(0).tolist() + ratio_trans_sust[:, 2, :].mean(0).tolist() + ratio_trans_sust[:, 3, :].mean(0).tolist()
    ...
    ax['regression'].spines['top'].set_visible(False)
    ax['regression'].spines['right'].set_visible(False)
    # ax['regression'].set_ylabel('R$_{transient}$:R$_{sustained}$', fontsize=fontsize_label)
    # ax['regression'].set_xlabel(r'R$^{2}_{lin}$:R$^{2}_{log}$', fontsize=fontsize_label)
    ax['regression'].set_xlim(lmts[0][0], lmts[0][1])
    ax['regression'].set_ylim(lmts[1][0], lmts[1][1])
...
def plot_regression_CEandSC(ratio_lin_log, ratio_trans_sust, CEandSC_values, n_layer, color, n_img, model, root):
    ...
    plt.savefig(root + 'onepulse_dynamics_regression_CEandSC.png', dpi=600)
    plt.savefig(root + 'onepulse_dynamics_regression_CEandSC.svg')
    plt.close()

def plot_regression_CEandSC_L1(ratio_lin_log, ratio_trans_sust, CEandSC_values, n_layer, color, n_img, model, root):
