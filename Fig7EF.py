...
# training set
# training_sets = ['Lnull_WT_VEN_fps3', 'Lnull_WT_VEN_fps6', 'Lnull_WT_VEN_fps12']
...
if preload == False:
        ...
        for iInit in range(init):
            ...
            for iL in range(n_layer):
                ...
                for i, datasetstim in enumerate(test_sets_stim):
                    ...
                    # retrieve data
                    # print(root_data + datasetstim + '_' + training_set + '_onepulse_' + output_mode + str(iL+1) + '_actvs_trained_' + str(iInit + 1) + '.npy')
                    temp = np.load(root_data + 'WT_VEN/' + datasetstim + '_' + training_set + '_onepulse_' + output_mode + str(iL+1) + '_actvs_trained_' + str(iInit + 1) + '.npy')
                    # print(temp.shape)
                    # print(temp[:, :int(n_img/2), :])
    ...
    ####################### SAVE
    np.save(root_data_save + 'loss_ratio_lin_log_' + test_sets, ratio_lin_log) 
    np.save(root_data_save + 'loss_ratio_trans_sust_' + test_sets, ratio_trans_sust)

# load data
ratio_lin_log = np.load(root_data_save + 'loss_ratio_lin_log_' + test_sets + '.npy') 
ratio_trans_sust = np.load(root_data_save + 'loss_ratio_trans_sust_' + test_sets + '.npy')

######################## VISUALIZE
plot_regression_all_loss_Fig7E(test_sets, ratio_lin_log, ratio_trans_sust, training_sets, n_layer, color_layer, n_img, init, root)
plot_regression_all_loss_Fig7F(test_sets, ratio_lin_log, ratio_trans_sust, training_sets, n_layer, color_layer, n_img, init, root)
